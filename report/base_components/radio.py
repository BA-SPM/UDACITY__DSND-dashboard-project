...
class Radio(BaseComponent):
    ...
    def build_component(self, entity_id, model):
        ...
        for value in self.values:
            input_child = Input(type="radio", id=value.lower(), name=self.name, value=value, hx_get=self.hx_get, hx_target=self.hx_target, checked=True if value == model.name.title() else None)
